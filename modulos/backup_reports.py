...
def get_secret(key, default=None):
    """Busca a chave nos secrets do Streamlit ou no arquivo .env sem gerar exceção."""
    try:
...
def listar_backups_csv(service, folder_id):
    """Mapeia todas as subpastas recursivamente no Drive e localiza qualquer arquivo CSV."""
    pastas_para_varrer = [folder_id]
    todas_pastas = [folder_id]

    while pastas_para_varrer:
        atual = pastas_para_varrer.pop(0)
        query_sub = f"'{atual}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false"
        resp = (
            service.files()
            .list(
                q=query_sub,
        ...
        novas = [p['id'] for p in resp.get('files', [])]
        pastas_para_varrer.extend(novas)
        todas_pastas.extend(novas)

    clausula_parents = ' or '.join([f"'{pid}' in parents" for pid in todas_pastas])
    query_csv = (
        f"({clausula_parents}) and mimeType='text/csv' and trashed=false"
    )
...
def formar_real(valor):
    """Converte valores numéricos ou strings brasileiras para o formato monetário R$ 0,00 sem corromper decimais."""
    if valor is None:
        return '0,00'
    try:
...
def converter_backup_csv_para_pdf(dados_csv, nome_periodo='HISTÓRICO'):
    ...
    elements = [
        Paragraph(
            f'<b>CONTROLE DE COMPRAS - {nome_periodo.upper()}</b>', title_style
        ),
    ...
    def style_th(text, bg_color):
        p_style = (
            header_yellow_style if bg_color == '#eab308' else header_style
        )
    ...
    cell_style = ParagraphStyle(
        'TD', fontSize=8, leading=10, fontName='Helvetica', alignment=1
    )
    cell_left = ParagraphStyle(
        'TDL', fontSize=8, leading=10, fontName='Helvetica-Bold', alignment=0
    )
    cell_bold = ParagraphStyle(
        'TDB', fontSize=8, leading=10, fontName='Helvetica-Bold', alignment=1
    )
    ...
    for row_idx, item in enumerate(dados_csv, start=1):
        ...
        st_text = (
            "<font color='#15803d'><b>Atingida</b></font>"
            if "Atingida" in st_meta and "Não" not in st_meta
            else "<font color='#dc2626'><b>Não Atingida</b></font>"
        )
        ...
        table_styles.append(
            ('BACKGROUND', (2, row_idx), (4, row_idx), colors.HexColor('#fef08a'))
        )
        table_styles.append(
            ('BACKGROUND', (5, row_idx), (5, row_idx), colors.HexColor('#d1fae5'))
        )
        table_styles.append(
            ('BACKGROUND', (6, row_idx), (6, row_idx), colors.HexColor('#dbeafe'))
        )
        table_styles.append(
            ('BACKGROUND', (7, row_idx), (7, row_idx), colors.HexColor('#fef08a'))
        )
        table_styles.append(
            ('BACKGROUND', (8, row_idx), (8, row_idx), colors.HexColor('#d1fae5'))
        )

    tot_row_idx = len(data)
    data.append([
        Paragraph(
            '<b>TOTAIS ACUMULADOS:</b>',
            ParagraphStyle(
                'TOTL',
                fontSize=8,
                leading=10,
        ...
        '',
        Paragraph(
            f'R$ {formar_real(tot_orcam_geral)}',
            ParagraphStyle(
                'TOTM',
                fontSize=8,
                leading=10,
        ...
        Paragraph(
            f'R$ {formar_real(tot_alvo_geral)}',
            ParagraphStyle(
                'TOTA',
                fontSize=8,
                leading=10,
        ...
        Paragraph(
            f'R$ {formar_real(tot_comprado_geral)}',
            ParagraphStyle(
                'TOTC',
                fontSize=8,
                leading=10,
        ...
        Paragraph(
            f'R$ {formar_real(tot_economia_geral)}',
            ParagraphStyle(
                'TOTE',
                fontSize=8,
                leading=10,
    ...
    table_styles.append(
        ('BACKGROUND', (0, tot_row_idx), (-1, tot_row_idx), colors.HexColor('#f1f5f9'))
    )
        ...
        if st.button('🔍 Buscar Backups Salvos no Drive'):
            try:
                ...
                if not creds_raw or not folder_id:
                    st.error(
                        "⚠️ As variáveis 'GOOGLE_DRIVE_CREDENTIALS' ou 'GOOGLE_DRIVE_FOLDER_ID' não foram encontradas!"
                    )
                else:
                    drive_service = obter_servico_drive(creds_raw)
                    arquivos = listar_backups_csv(drive_service, folder_id)

                    if arquivos:
                        st.session_state.lista_backups = {
                            arq['name']: arq['id'] for arq in arquivos
                        }
                    else:
                        st.info(
                            'Nenhum arquivo de backup CSV encontrado na pasta do Google Drive.'
                        )
            except Exception as e:
                st.error(f'Erro ao acessar o Drive: {e}')

        if (
            'lista_backups' in st.session_state
            and st.session_state.lista_backups
        ):
            arq_selecionado = st.selectbox(
                'Selecione o backup desejado:',
                list(st.session_state.lista_backups.keys()),
            )

            if st.button('📄 Gerar PDF deste Backup'):
                with st.spinner('Lendo arquivo do Drive e gerando PDF...'):
                    try:
                        creds_raw = get_secret('GOOGLE_DRIVE_CREDENTIALS')
                        drive_service = obter_servico_drive(creds_raw)

                        file_id = st.session_state.lista_backups[
                            arq_selecionado
                        ]
                        dados_csv = ler_csv_drive(drive_service, file_id)
                        pdf_bytes = converter_backup_csv_para_pdf(
                            dados_csv,
                            nome_periodo=arq_selecionado.replace('.csv', ''),
                        )

                        st.success('✅ PDF do backup antigo gerado com sucesso!')
                        st.download_button(
                            label='📥 Baixar PDF do Backup no Computador',
                            data=pdf_bytes,
                            file_name=f'Relatorio_{arq_selecionado}.pdf',
                            mime='application/pdf',
                        )
                    except Exception as err_pdf:
                        st.error(
                            f'Ocorreu um erro ao processar o arquivo CSV: {err_pdf}'
                        )